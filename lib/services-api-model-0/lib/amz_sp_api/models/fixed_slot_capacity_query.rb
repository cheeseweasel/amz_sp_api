  ...
  class AmzSpApi::ServicesApiModel::V0::FixedSlotCapacityQuery
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::ServicesApiModel::FixedSlotCapacityQuery` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::ServicesApiModel::FixedSlotCapacityQuery`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'capacity_types')
        if (value = attributes[:'capacity_types']).is_a?(Array)
          self.capacity_types = value
        end
      end

      if attributes.key?(:'slot_duration')
        self.slot_duration = attributes[:'slot_duration']
      end
    ...
    def hash
      [capacity_types, slot_duration, start_date_time, end_date_time].hash
    end
