...
module AmzSpApi::VendorShipmentsApiModel
  ...
  class AmzSpApi::VendorShipmentsApiModel::V0::PackedItems
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::VendorShipmentsApiModel::PackedItems` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::VendorShipmentsApiModel::PackedItems`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'item_details')
        self.item_details = attributes[:'item_details']
      end
    end
    ...
    def hash
      [item_sequence_number, buyer_product_identifier, vendor_product_identifier, packed_quantity, item_details].hash
    end
