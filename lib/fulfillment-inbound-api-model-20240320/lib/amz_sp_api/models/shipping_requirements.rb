...
module AmzSpApi::FulfillmentInboundApiModel
  ...
  class AmzSpApi::FulfillmentInboundApiModel::V20240320::ShippingRequirements
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::FulfillmentInboundApiModel::ShippingRequirements` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::FulfillmentInboundApiModel::ShippingRequirements`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'modes')
        if (value = attributes[:'modes']).is_a?(Array)
          self.modes = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @modes.nil?
        invalid_properties.push('invalid value for "modes", modes cannot be nil.')
      end
