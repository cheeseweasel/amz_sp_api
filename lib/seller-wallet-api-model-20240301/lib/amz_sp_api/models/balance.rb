...
module AmzSpApi::SellerWalletApiModel
  ...
  class AmzSpApi::SellerWalletApiModel::V20240301::Balance
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::SellerWalletApiModel::Balance` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::SellerWalletApiModel::Balance`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @balance_currency.nil?
        invalid_properties.push('invalid value for "balance_currency", balance_currency cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @balance_currency.nil?
    ...
    def hash
      [account_id, balance_type, balance_amount, balance_currency, last_update_date].hash
    end
