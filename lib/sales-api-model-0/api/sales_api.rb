...
module AmzSpApi::SalesApiModel::V0
  class SalesApi
    ...
    def initialize(api_client = AmzSpApi::SalesApiModel::V0::ApiClient.default)
    ...
    # @option opts [String] :amazon_program Filters the results by the Amazon program that you specify. Do not include this filter if you want the response to include order metrics for all programs. **Example:** &#x60;AmazonHaul&#x60; returns order metrics for the Amazon Haul program only.
    ...
    def get_order_metrics_with_http_info(marketplace_ids, interval, granularity, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'amazon_program'] && !['AmazonHaul'].include?(opts[:'amazon_program'])
        fail ArgumentError, 'invalid value for "amazon_program", must be one of AmazonHaul'
      end
      # resource path
      ...
      query_params[:'amazonProgram'] = opts[:'amazon_program'] if !opts[:'amazon_program'].nil?
