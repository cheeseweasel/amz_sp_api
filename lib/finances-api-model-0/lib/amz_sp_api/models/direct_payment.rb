...
module AmzSpApi::FinancesApiModel
  ...
  class AmzSpApi::FinancesApiModel::V0::DirectPayment
    ...
    def self.openapi_types
      {
        :'direct_payment_type' => :'Object',
        :'direct_payment_amount' => :'Object'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::FinancesApiModel::DirectPayment` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::FinancesApiModel::DirectPayment`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'direct_payment_type')
        self.direct_payment_type = attributes[:'direct_payment_type']
      end

      if attributes.key?(:'direct_payment_amount')
        self.direct_payment_amount = attributes[:'direct_payment_amount']
      end
    end
