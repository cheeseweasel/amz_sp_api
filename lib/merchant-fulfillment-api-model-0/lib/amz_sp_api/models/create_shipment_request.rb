...
module AmzSpApi::MerchantFulfillmentApiModel
  # Request schema.
  class AmzSpApi::MerchantFulfillmentApiModel::V0::CreateShipmentRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::MerchantFulfillmentApiModel::CreateShipmentRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::MerchantFulfillmentApiModel::CreateShipmentRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'shipment_request_details')
        self.shipment_request_details = attributes[:'shipment_request_details']
      end
      ...
      if attributes.key?(:'hazmat_type')
        self.hazmat_type = attributes[:'hazmat_type']
      end

      if attributes.key?(:'label_format_option')
        self.label_format_option = attributes[:'label_format_option']
      end

      if attributes.key?(:'shipment_level_seller_inputs_list')
