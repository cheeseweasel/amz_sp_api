...
module AmzSpApi::VehiclesApiModel
  class AmzSpApi::VehiclesApiModel::V20241101::AutomotiveApi
    ...
    def get_vehicles_with_http_info(marketplace_id, vehicle_type, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: AutomotiveApi.get_vehicles ...'
      end
      # verify the required parameter 'marketplace_id' is set
      if @api_client.config.client_side_validation && marketplace_id.nil?
        fail ArgumentError, "Missing the required parameter 'marketplace_id' when calling AutomotiveApi.get_vehicles"
      end
      # verify the required parameter 'vehicle_type' is set
      if @api_client.config.client_side_validation && vehicle_type.nil?
        fail ArgumentError, "Missing the required parameter 'vehicle_type' when calling AutomotiveApi.get_vehicles"
      end
      # verify enum value
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: AutomotiveApi#get_vehicles\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
