...
module AmzSpApi::ShippingApiModel
  ...
  class AmzSpApi::ShippingApiModel::V0::AvailableValueAddedServiceGroup
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AmzSpApi::ShippingApiModel::AvailableValueAddedServiceGroup` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AmzSpApi::ShippingApiModel::AvailableValueAddedServiceGroup`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'value_added_services')
        if (value = attributes[:'value_added_services']).is_a?(Array)
          self.value_added_services = value
        end
      end
    end
    ...
    def hash
      [group_id, group_description, is_required, value_added_services].hash
    end
